...
.actions button.is-selected{outline:2px solid var(--amber);outline-offset:2px;
  box-shadow:0 0 0 1px var(--amber);font-weight:700;transform:translateY(-1px)}
.actions button.confirm.is-selected{background:#1a3a28;color:#b8f0cf}
.mine-box{margin:10px 0 0;padding:12px 14px;border:1px solid rgba(229,160,13,.45);
  border-radius:4px;background:rgba(229,160,13,.08);font-family:ui-monospace,Menlo,monospace;font-size:14px}
.mine-box b{color:var(--amber);font-size:16px;letter-spacing:.04em;text-transform:uppercase}
.mine-box.empty{border-color:var(--line);background:transparent;color:var(--dim);font-size:13px}
.mine-box.empty b{color:var(--dim);font-size:inherit;letter-spacing:0;text-transform:none}
  ...
  <div class="stage">
    ...
    <div class="panel">
      ...
      <div class="actions">
        ...
        <button type="button" class="painted" id="btnPainted" data-pick="painted" title="1">1 · painted</button>
        <button type="button" class="photo" id="btnPhoto" data-pick="photo" title="2">2 · photo</button>
        <button type="button" class="composite" id="btnComposite" data-pick="composite" title="3">3 · composite</button>
        <button type="button" class="doubt" id="btnDoubt" data-pick="doubtful" title="4">4 · doubtful</button>
        <button type="button" class="nav" id="btnPrev" title="←">← Prev</button>
        <button type="button" class="nav" id="btnNext" title="→">Next →</button>
      </div>
      <div class="mine-box empty" id="mineBox">Tu selección r2: <b>—</b></div>
...
function show(){
  ...
  const mine = document.getElementById("mineBox");
  // highlight the button that matches the saved r2 choice
  document.querySelectorAll(".actions button[data-pick], #btnConfirm").forEach(b => b.classList.remove("is-selected"));
  if(!v){
    verd.textContent = "Sin revisión r2 — elegí confirmar o cambiar";
    verd.className = "verdict";
    mine.className = "mine-box empty";
    mine.innerHTML = "Tu selección r2: <b>todavía no marcada</b>";
  } else {
    const same = v.pass2===d.pass1;
    mine.className = "mine-box";
    mine.innerHTML = "Tu selección r2: <b>" + v.pass2 + "</b>" +
      (same ? " · confirmaste pass1" : " · cambiaste desde <b>" + d.pass1 + "</b>");
    if(same){
      verd.textContent = "Guardado: confirmada → " + v.pass2;
      verd.className = "verdict ok";
      document.getElementById("btnConfirm").classList.add("is-selected");
    } else {
      verd.textContent = "Guardado: cambiada " + d.pass1 + " → " + v.pass2;
      verd.className = "verdict " + v.pass2;
    }
    const pick = document.querySelector('.actions button[data-pick="'+v.pass2+'"]');
